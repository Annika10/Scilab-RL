...
def train_function(config) -> None:
    ...
    if torch.cuda.is_available():
        ...
        if torch.cuda.device_count() > 1:
            fm_network = nn.DataParallel(fm_network)
    fm_network.to(device)
    ...
    if train.get_checkpoint():
        ...
        with loaded_checkpoint.as_directory() as loaded_checkpoint_dir:
            ...
            fm_network.load_state_dict(model_state)
            fm_optimizer.load_state_dict(optimizer_state)
    ...
    for epoch in range(20000):
        ...
        with tempfile.TemporaryDirectory() as temp_checkpoint_dir:
            ...
            torch.save(
                (fm_network.state_dict(), fm_optimizer.state_dict()), path
            )
