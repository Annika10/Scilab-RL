...
def train_fm(observations: torch.Tensor, actions: torch.Tensor, rewards: torch.Tensor, reward_predicting: bool,
             fm_network, fm_optimizer, maximum_number_of_objects, best_loss, criterion) -> tuple[float, float]:
    """
    ...
    loss = fw_loss.mean()
    # loss = criterion(forward_model_prediction_normal_distribution, next_observations_formatted)
    wandb.log({"loss": loss})
...
def train(reward_predicting, fm_network, fm_optimizer, maximum_number_of_objects, criterion) -> None:
    """
...
train(reward_predicting=reward_predicting, fm_network=fm_network,
      fm_optimizer=fm_optimizer, maximum_number_of_objects=maximum_number_of_objects, criterion=criterion)
