...
class TestMetaEnvPretrainedWithoutSoC(unittest.TestCase):
    ...
    def test_different_situations_with_input_noise_in_one_task(self, input_noise_mock,
        ...
        env.reset()
        with self.subTest("three steps action 0, with input noise"):
            state, reward, is_done, _, info = env.step(0)
        ...
        with self.subTest("switch to action 1, without input noise"):
            state, reward, is_done, _, info = env.step(1)
            
            ### active task
            # actual state
            # agent: x position: 25, go to the right --> 27
            # all objects: y position -1 + new incoming objects
            new_state_active = np.array([27, 1, 30, 0, 27, 5, 16, 11, 12, 16, 24, 22, 7, 25, 16, 30, 0, 0, 0, 0, 0, 0])
            # we cannot see the belief state of the active task
            # from 22 to 44 elements because of action 1
            np.testing.assert_array_equal(new_state_active, state[22:])
        ...
        with self.subTest("three steps action 1, without input noise"):
            state, reward, is_done, _, info = env.step(1)
            
            ### active task
            # actual state
            # agent: x position: 27, go to the right --> 29
            # all objects: y position -1 + new incoming objects
            new_state_active = np.array([29, 1, 30, -1, 27, 4, 16, 10, 12, 15, 24, 21, 7, 24, 16, 29, 0, 0, 0, 0, 0, 0])
            # we cannot see the belief state of the active task
            # from 22 to 44 elements because of action 1
            np.testing.assert_array_equal(new_state_active, state[22:])
            ...
            state, reward, is_done, _, info = env.step(1)
            self.assertEqual(round(reward, ndigits=0), 0)
            
            ### active task
            # actual state
            # agent: x position: 29, go to the right --> 31
            # all objects: y position -1 + new incoming objects
            new_state_active = np.array([31, 1, 27, 3, 16, 9, 12, 14, 24, 20, 7, 23, 16, 28, 0, 0, 0, 0, 0, 0, 0, 0])
            # we cannot see the belief state of the active task
            # from 22 to 44 elements because of action 1
            np.testing.assert_array_equal(new_state_active, state[22:])
            ...
            ### active task
            # actual state
            # agent: x position: 31, go to the right --> 33
            # all objects: y position -1 + new incoming objects
            new_state_active = np.array([33, 1, 27, 2, 16, 8, 12, 13, 24, 19, 7, 22, 16, 27, 16, 28, 0, 0, 0, 0, 0, 0])
            # we cannot see the belief state of the active task
            # from 22 to 44 elements because of action 1
            np.testing.assert_array_equal(new_state_active, state[22:])
        ...
        with self.subTest("switch to action 0, with input noise"):
            state, reward, is_done, _, info = env.step(0)
            self.assertEqual(round(reward, ndigits=0), 0)
            
            ### active task
            # actual state
            # agent: x position: 23, go to the right --> 25 + input noise of -2 --> 23
            # all objects: y position -1 + new incoming objects
            new_state_active = np.array([23, 1, 28, 2, 20, 6, 25, 13, 35, 15, 39, 20, 26, 25, 0, 0, 0, 0, 0, 0, 0, 0])
            # we cannot see the belief state of the active task
            # first 22 elements because of action 0
            np.testing.assert_array_equal(new_state_active, state[:22])
            
            ### inactive task belief state:
            # agent: x position: 33, go one step down --> 33 without input noise
            # all objects: y position -1, no new incoming objects
            new_state_inactive = np.array(
                [33, 1, 27, 1, 16, 7, 12, 12, 24, 18, 7, 21, 16, 26, 16, 27, 0, 0, 0, 0, 0, 0])
            # we cannot see the actual state of the inactive task
            # from 22 to 44 elements because of action 0
            np.testing.assert_array_equal(new_state_inactive, state[22:])
